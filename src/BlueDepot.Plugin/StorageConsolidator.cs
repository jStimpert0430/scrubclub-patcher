...
internal sealed class StorageConsolidator:MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        view.Register<string>(Job,(sender,id)=>
        {
            ...
            if(depot && Plugin.IsDepot(depot) && Vector3.Distance(chest.transform.position,depot.transform.position)<=Plugin.Radius.Value)
                view.GetZDO().Set(Job,id);
        });
    }
    internal void Request(Container depot)=>view.InvokeRPC(Job,Storage.Id(depot));
    void Update()
    {
        ...
        var depot=Storage.Find(id);if(!depot || !Storage.CanAccess(depot))return;
